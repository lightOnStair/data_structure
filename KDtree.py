...
def comp(cur,p,tar):
    if cur.dist(tar) <= p.dist(tar):
        return cur
    else:
        return p

def closer(root,best,tar):
    curbest = root
    if root == None:
        return best
    while root.left != None and root.right != None :
        if root.depth % 2 == 0:
            ...
            if tar.y < root.y:
                ...
                root = root.right
        curbest = comp(curbest,root,tar)
    curbest = comp(curbest,best,tar)
    return curbest


def nearest_neighbor(kd_tree,tar):
    def find(curbest,tar):
        prev = curbest
        if curbest.depth % 2 == 0:
            if tar.x < curbest.x:
                if curbest.dist(tar) > abs(curbest.x - tar.x):
                    curbest = closer(curbest.left,curbest,tar)
            else:
                if curbest.dist(tar) > abs(curbest.x - tar.x):
                    curbest = closer(curbest.right,curbest,tar)
        else:
            if tar.y < curbest.y:
                if curbest.dist(tar) > abs(curbest.y - tar.y):
                    curbest = closer(curbest.left,curbest,tar)
            else:
                if curbest.dist(tar) > abs(curbest.x - tar.x):
                    curbest = closer(curbest.right,curbest,tar)
        if prev != curbest:
            curbest = find(curbest,tar)
            prev = curbest
        temp = curbest.parent
        cur = curbest
        while temp != None:
            if cur == temp.right:
                if curbest.dist(tar) > abs(temp.x - tar.x):
                    curbest = closer(temp.left,curbest,tar)
            else:
                if curbest.dist(tar) > abs(temp.y - tar.y):
                    curbest = closer(temp.left,curbest,tar)
            if prev != curbest:
                curbest = find(curbest,tar)
            cur = cur.parent
    ...
    if kd_tree.root == None:
        return None
    curbest = closer(kd_tree.root,kd_tree.root,tar)
    return find(curbest,tar)
