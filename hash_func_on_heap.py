...
def database_search(database,field,value):
    res = []
    for robot in getattr(database, field+'_table')[hash_function(value)]:
...
def database_delete(database,field,value):
    needDel = database_search(database,field,value)
    for robot in needDel:
        for target in database.name_table[hash_function(robot.name)]:
            if target == robot:
                database.name_table[hash_function(robot.name)].remove(target)
        for target in database.shape_table[hash_function(robot.shape)]:
            if target == robot:
                database.shape_table[hash_function(robot.shape)].remove(target)
        for target in database.color_table[hash_function(robot.color)]:
            if target == robot:
                database.color_table[hash_function(robot.color)].remove(target)
